...
def execute_query_returning_id(db: PostgresDatabase, query: str, params: Tuple = ()) -> int:
...
def start_execution(db: PostgresDatabase, execution_id: str, total_configs_executed: int, start_time: datetime) -> int:
    ...
    return execute_query_returning_id(db, query, (execution_id, total_configs_executed, initial_status, start_time))

def end_execution(db: PostgresDatabase, run_id: int, successful_configs: int, failed_configs: int, status: str, end_time: datetime, error_message: Optional[str] = None):
    ...
    """
    execute_query(db, query, (successful_configs, failed_configs, status, end_time, error_message, run_id))

def log_step_detail(db: PostgresDatabase, run_id: int, execution_id: str, system_config_id: int, system_name: str, partner_name: str,
                    step: str, status: str, records_fetched: int = 0, records_success: int = 0, 
                    records_error: int = 0, error_message: Optional[str] = None, 
...
def fetch_system_configurations(
    ...
    configs = [
        SystemConfiguration(
            ...
            config=row.get("config") if isinstance(row.get("config"), dict) else json.loads(row.get("config")),
