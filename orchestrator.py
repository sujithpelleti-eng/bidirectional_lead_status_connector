...
class Orchestrator:
    def __init__(self, schedule: Optional[str] = None, system: Optional[str] = None, partner_id: Optional[str] = None, from_date: Optional[str] = None, to_date: Optional[str] = None):
        ...
        logger.info(f"Orchestrator initialized with schedule={schedule}, system={system}, partner_id={partner_id}, from_date={from_date}, to_date={to_date}")
    ...
    def process_configuration(self, config):
        ...
        try:
            ...
            raw_data = connector.fetch_raw_data(from_date=self.from_date, to_date=self.to_date)
            ...
            for destination in destinations:
                if isinstance(destination, S3Destination):
                    ...
                    destination.send(raw_data, provider=config.system_name, partner_id=config.partner_id, file_type=config.file_type)
                    log_step_detail(
                        db=self.db,
                        run_id=self.run_id,
                        execution_id=self.execution_id,
            ...
            error_message = str(e)
            logger.error(f"Error processing config {config.system_name}-{config.partner_name}: {error_message}")
            log_step_detail(
                db=self.db,
                run_id=self.run_id,
                execution_id=self.execution_id,
    ...
    def run(self):
        ...
        try:
            logger.info(f"Starting the execution with execution_id: {self.execution_id}")
            ...
            for config in configurations:
                try:
                    logger.info(f"Processing configuration: {config.system_name}-{config.partner_name}")
                    ...
                    if self.process_configuration(config):
                        ...
                        failure_count += 1
                        error_messages.append(f"Failed configuration: {config.system_name}-{config.partner_name}")
                
                except Exception as e:
                    # Log and track any unexpected exceptions for the configuration
                    error_message = str(e)
                    logger.error(f"Unexpected error for configuration {config.system_name}-{config.partner_name}: {error_message}")
                    failure_count += 1
                    error_messages.append(f"{config.system_name}-{config.partner_name} error: {error_message}")
            ...
            self.end_execution(success_count, failure_count, error_message=error_summary)
            ...
            self.end_execution(success_count=0, failure_count=len(configurations), error_message=error_summary)
